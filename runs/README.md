...
## 新增：GE 八组服务器消融（50 轮，seed 2027）

[八组结果、逐轮指标与续跑耗时](server_ge50_seed2027/README.md)已归档。
方格全4/半2方向、Hex 全6/半3方向，分别对比单/多尺度；全部完成50轮。
这是约133万参数的显式方向 GE 骨干，与下方约550万参数的 ViT 系列分开比较。
最佳为 Hex 全6方向单尺度：Top-1 63.08%，对应 Top-5 86.70%；当前多尺度实现四组均低于单尺度。
目录保留实际配置、400条逐轮指标、参数统计、运行环境、续跑记录与来源校验值；不含权重或数据集。
