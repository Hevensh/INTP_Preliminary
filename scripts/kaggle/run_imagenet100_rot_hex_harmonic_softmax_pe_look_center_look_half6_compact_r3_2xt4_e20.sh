...
EPOCHS="${EPOCHS:-20}"
NAME="deit_tiny_rot_hex_harmonic_softmax_pe_look_center_look_half6_compact_r3_imagenet100_ddp_e20"
...
echo "[run] ${EPOCHS} epochs | Hex half6d3r + null-softmax | PE + image Look + Center Look | batch ${BATCH_SIZE}/GPU"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/deit_tiny_rot_hex_harmonic_softmax_pe_look_center_look_half6_compact_r3_ddp_e20.json \
