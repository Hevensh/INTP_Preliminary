...
NAME="deit_tiny_rot_hex_harmonic_softmax_pe_half3_compact_r3_imagenet100_ddp_e5"
...
echo "[run] 5 epochs | half3d3r + null-softmax | PE only | no differentiation | batch ${BATCH_SIZE}/GPU"
echo "[poses] 3 half-circle directions | global grid 6 | K24/K12 | 6 real poses + null"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/deit_tiny_rot_hex_harmonic_softmax_pe_half3_compact_r3_ddp_e5.json \
