...
EPOCHS="${EPOCHS:-5}"
NAME="${EXPERIMENT_NAME:-resnet18_multiscale_rotconv4_k5k3_imagenet100_ddp_e${EPOCHS}}"
...
echo "[run] ResNet-18 multi-scale rotating conv | K5/K3 | half4 directions | ${EPOCHS} epochs | batch ${BATCH_SIZE}/GPU"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/resnet18_multiscale_rotconv4_ddp_e20.json \
