...
EPOCHS="${EPOCHS:-20}"
NAME="deit_tiny_rot_hex_harmonic_softmax_look_half6_imagenet100_ddp_e20"
...
echo "[run] Rot-Hex half6/12 null-softmax | half6/12 null-softmax Look only | batch ${BATCH_SIZE}/GPU"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/deit_tiny_rot_hex_harmonic_softmax_look_half6_ddp_e20.json \
