...
EPOCHS="${EPOCHS:-5}"
NAME="${EXPERIMENT_NAME:-resnet18_fixed_rotinterp8_k3_imagenet100_ddp_e${EPOCHS}}"
...
echo "[run] ResNet-18 fixed rotation | full-circle bilinear D8 K3 | ${EPOCHS} epochs | batch ${BATCH_SIZE}/GPU"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/resnet18_fixed_rotinterp8_ddp_e20.json \
