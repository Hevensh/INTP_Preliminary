...
NAME="deit_tiny_rot_hex_dot_simple_pe_imagenet100_ddp_e20"
...
echo "[run] Rot-Hex Full-4 dot soft-pool + Linear(96,192) + PE | null -1"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/deit_tiny_rot_hex_dot_simple_pe_ddp_e20.json \
  --data-root "${DATA_ROOT}" \
