...
EPOCHS="${EPOCHS:-20}"
NAME="deit_tiny_rot_hex_harmonic_softmax_look_imagenet100_ddp_e20"
...
echo "[run] Rot-Hex dot null-softmax | null-softmax Look only | batch ${BATCH_SIZE}/GPU"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/deit_tiny_rot_hex_harmonic_look_ddp_e20.json \
