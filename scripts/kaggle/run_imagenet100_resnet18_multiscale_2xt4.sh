...
EPOCHS="${EPOCHS:-5}"
NAME="${EXPERIMENT_NAME:-resnet18_multiscale_k5k3_imagenet100_ddp_e${EPOCHS}}"
...
echo "[run] ResNet-18 multi-scale | K5/K3 | ${EPOCHS} epochs | batch ${BATCH_SIZE}/GPU"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/resnet18_multiscale_ddp_e20.json \
