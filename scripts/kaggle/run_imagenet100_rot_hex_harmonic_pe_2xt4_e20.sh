...
NAME="deit_tiny_rot_hex_harmonic_pe_imagenet100_ddp_e20"
...
echo "[run] Rot-Hex linear harmonic | 96x(cos,sin) | K24+K12 | PE"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/deit_tiny_rot_hex_harmonic_pe_ddp_e20.json \
  --data-root "${DATA_ROOT}" \
