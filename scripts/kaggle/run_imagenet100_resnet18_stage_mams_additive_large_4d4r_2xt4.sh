...
EPOCHS="${EPOCHS:-5}"
NAME="${EXPERIMENT_NAME:-resnet18_stage_mams_additive_large_4d4r_imagenet100_ddp_e${EPOCHS}}"
...
echo "[run] Additive Stage-MAMS ResNet-18 | keep both 3x3 convs | half4d4r | ${EPOCHS} epochs | batch ${BATCH_SIZE}/GPU"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/resnet18_stage_mams_additive_large_4d4r_ddp_e20.json \
