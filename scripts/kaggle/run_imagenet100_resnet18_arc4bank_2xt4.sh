...
echo "[run] ResNet-18 ARC | 4 adaptive kernel banks, bilinear K3 | ${EPOCHS} epochs | batch ${BATCH_SIZE}/GPU"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/resnet18_arc4bank_ddp_e20.json \
