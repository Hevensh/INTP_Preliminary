#!/usr/bin/env bash
set -euo pipefail
echo "[run] full12r3 | 192 bases | moments 1+2 | four groups SUM ->192 | PE only | 20 epochs"
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE:-2}" \
  -m experiments.imagenet100.train_vit \
  --config configs/imagenet100/rot_hex_full12r3_moments12_b192_sum4_pe_ddp_e20.json \
