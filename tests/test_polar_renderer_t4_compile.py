...
@pytest.mark.parametrize('directions,period,resolution',[(11,23,7),(7,14,5)])
def test_t4_backward_shared_memory_budget(directions,period,resolution):
    ...
    model = HexRotatingHarmonicPatchEmbed(img_size=224,in_chans=3,embed_dim=192,
        directions=directions,global_directions=period,angular_bins_per_radius=resolution)
    for renderer in model.renderers:
        contributions=renderer.reverse_lookup.shape[1]
        width=triton.next_power_of_2(contributions)
        stored=resolution*78
        constants=dict(elements=16*3*stored,channels=3,directions=directions,
            samples=renderer.index_r0_a0.shape[1],stored=stored,
