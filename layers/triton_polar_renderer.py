...
if triton is not None:
    ...
    @triton.jit
    def _polar_backward(
        grad_output, i00, i01, i10, i11, a0, a1, radial, grad_prototype,
        elements: tl.constexpr, channels: tl.constexpr,
        directions: tl.constexpr, samples: tl.constexpr,
        stored: tl.constexpr, block: tl.constexpr,
    ):
        ...
        angle1 = tl.load(a1 + lookup, mask=mask, other=0.0).to(tl.float32)
        radius = tl.load(radial + sample, mask=mask, other=0.0).to(tl.float32)
        grad = tl.load(grad_output + offset, mask=mask, other=0.0).to(tl.float32)
        origin = (base * channels + channel) * stored
        tl.atomic_add(
            grad_prototype + origin + index00,
            grad * (1.0 - radius) * (1.0 - angle0), mask=mask,
        )
        tl.atomic_add(
            grad_prototype + origin + index01,
            grad * (1.0 - radius) * angle0, mask=mask,
        )
        tl.atomic_add(
            grad_prototype + origin + index10,
            grad * radius * (1.0 - angle1), mask=mask,
        )
        tl.atomic_add(
            grad_prototype + origin + index11,
            grad * radius * angle1, mask=mask,
        )


class _TritonPolarRender(torch.autograd.Function):
    @staticmethod
    def forward(ctx, prototype, i00, i01, i10, i11, a0, a1, radial):
        ...
        ctx.save_for_backward(i00, i01, i10, i11, a0, a1, radial)
    ...
    def backward(ctx, grad_output):
        i00, i01, i10, i11, a0, a1, radial = ctx.saved_tensors
        ...
        block = 256
        _polar_backward[(triton.cdiv(elements, block),)](
            grad_output.contiguous(), i00, i01, i10, i11, a0, a1, radial,
            grad_prototype, elements=elements, channels=channels,
            directions=directions, samples=samples, stored=stored, block=block,
        )
        return grad_prototype, None, None, None, None, None, None, None
